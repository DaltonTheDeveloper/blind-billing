...
import { dynamo, TABLES, PutCommand, QueryCommand, UpdateCommand, GetCommand } from "../shared/dynamo.js";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  if (path.includes("/mock-pay/")) {
    ...
    // Query payment_id-index to find the transaction
    ...
    await dynamo.send(new UpdateCommand({
      TableName: TABLES.transactions,
      Key: { id: txn.id },
      UpdateExpression: "SET #status = :status, card_brand = :brand, card_last4 = :last4, settled_at = :settled",
      ExpressionAttributeNames: { "#status": "status" },
      ExpressionAttributeValues: {
        ":status": "paid",
        ":brand": webhook.card_brand,
        ":last4": webhook.card_last4,
  ...
  // Query kurv_payment_id-index to find the transaction
  const { Items } = await dynamo.send(new QueryCommand({
    TableName: TABLES.transactions,
  ...
  await dynamo.send(new UpdateCommand({
    ...
    ExpressionAttributeValues: {
      ":status": newStatus,
      ":brand": body.card_brand ? String(body.card_brand) : null,
      ":last4": body.card_last4 ? String(body.card_last4) : null,
      ":settled": newStatus === "paid" ? new Date().toISOString() : null,
    },
  }));
